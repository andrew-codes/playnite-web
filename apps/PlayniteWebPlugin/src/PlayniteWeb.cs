...
namespace PlayniteWeb
{
    public class PlayniteWeb : GenericPlugin
    {
        ...
        public PlayniteWeb(IPlayniteAPI api) : base(api)
        {
            ...
            // Subscribe to disconnection events to handle reconnections
        ...
        private async void StartConnection(PlayniteWebSettings settings)
        {
            ...
            try
            {
                await publisher.StartConnectionAsync(options);
                Logger.Info("Connected to MQTT server.");
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "Error occurred during initial connection. Starting reconnection attempts.");
            ...
            while (!publisher.IsConnected)
            {
                try
                {
                    ...
                    if (publisher.IsConnected)
                    {
                        ...
                        await ResubscribeToTopics(); // After reconnection, resubscribe to necessary topics
                        break; // Exit loop if reconnected
                    }
                }
                catch (Exception ex)
                {
        ...
        private async Task ResubscribeToTopics()
        {
            try
            {
                // Example of resubscribing to topics
                var subscribeOptions = new MqttClientSubscribeOptionsBuilder()
                    .WithTopicFilter(f => f.WithTopic(topicManager.GetPublishTopic(PublishTopics.Connection())))
                    .Build();

                await publisher.SubscribeAsync(subscribeOptions);
                Logger.Info("Successfully resubscribed to topics.");
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "Failed to resubscribe to topics.");
            }
        }
