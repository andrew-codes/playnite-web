...
{
  public class PublishReleaseCollectionGraphQL : IPublishCollectionsToPlayniteWeb<IIdentifiable>
  {
    ...
    public PublishReleaseCollectionGraphQL(GraphQLHttpClient gql, IGameDatabaseAPI db, string deviceId, PlayniteWebSettings settings)
    {
      this.gql = gql;
      this.db = db;
      ...
      yield return gql.SendMutationAsync<dynamic>(new GraphQLHttpRequest
      {
        ...
        Variables = new
        {
          libraryData = new
          {
            ...
            remove = new
            {
              releases = removed.Select(r => r.Id),
