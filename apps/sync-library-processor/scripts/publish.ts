...
async function run() {
  ...
  // Create and push manifest for each version tag
  // This preserves all architectures (linux/amd64 and linux/arm64)
  await Promise.all(
    tags.map(
      (tag) =>
        new Promise((resolve, reject) => {
          ...
          const child = sh.exec(
            `docker buildx imagetools create -t ${targetImage} ${sourceImage}`,
            {
              async: true,
              silent: false,
            },
          ...
          child.on('exit', (code) => {
            if (code !== 0) {
              return reject(
                new Error(
                  `Manifest creation and push failed with exit code ${code}`,
                ),
              )
            }
            return resolve(code)
